...
class AddressViewModel @Inject constructor(
    ...
    fun getCities() {
        viewModelScope.launch {
            repository.getCities(token).catch { e -> e.printStackTrace() }
    ...
    fun getDistrict(cityId: Int?) {
        viewModelScope.launch {
            repository.getDistrict(token, CityIdRequest(cityId))
    ...
    fun addAddress(
        ...
        viewModelScope.launch {
            ...
            try {
                repository.addAddress(token, request)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
    ...
    fun getAddresses(userId: String?) {
        viewModelScope.launch {
            repository.getAddresses(token, GetAddressRequest(userId))
                .catch { e -> e.printStackTrace() }
    ...
    fun updateAddress(
        ...
        viewModelScope.launch {
            ...
            try {
                repository.updateAddress(token, request)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
